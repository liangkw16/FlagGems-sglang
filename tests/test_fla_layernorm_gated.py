...
def reference(x, g, weight, bias, activation="swish", eps=1e-5,
              is_rms_norm=True):
    out_dtype = x.dtype
    xf = x.float()
    if is_rms_norm:
...
class FlaLayernormGatedTest(unittest.TestCase):
    def _check(self, x, g, weight=None, bias=None, activation="swish",
               eps=1e-5, is_rms_norm=True):
    ...
    def test_weight_bias_combos(self):
        ...
        for has_w, has_b in ((True, True), (True, False), (False, True), (False, False)):
