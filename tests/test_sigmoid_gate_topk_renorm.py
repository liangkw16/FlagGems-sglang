...
MODULES = load_operator_modules("sigmoid_gate_topk_renorm")
...
class TestSigmoidGateTopkRenorm(unittest.TestCase):
    ...
    def _run(self, module, T, N, S, k, dtype, seed=0):
        ...
        rw, idx, sw = module.sigmoid_gate_topk_renorm(
            logits, k, S, 2.5, gs, bias
        )
        ...
        for name, module in MODULES:
            for dtype in (torch.float32, torch.float16, torch.bfloat16):
                for T, N, S, k in [
                    ...
                    with self.subTest(
                        module=name, dtype=dtype, T=T, N=N, S=S, k=k
                    ):
                        self._run(module, T, N, S, k, dtype)
    ...
    def test_scale_tensor_and_invariance(self):
        ...
        for name, module in MODULES:
            with self.subTest(module=name):
                scalar = module.sigmoid_gate_topk_renorm(
                    logits, 4, 2, 1.0, 2.0, bias
                )
                tensor = module.sigmoid_gate_topk_renorm(
                    logits,
                    4,
                    2,
                    1.0,
    ...
    def test_row_grid_fold(self):
        for name, module in MODULES:
            with self.subTest(module=name):
                self._run(module, 70000, 8, 1, 2, torch.float16)
