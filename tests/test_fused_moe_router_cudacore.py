...
class FusedMoeRouterCudacoreTest(unittest.TestCase):
    ...
    def test_near_tie_order(self):
        ...
        w[5, 0] = 4.0 - 1e-3
        w[2, 0] = 3.0
        ids = self._check(x, w, 4, 0.0, None)
        for row in range(4):
            self.assertEqual(ids[row, 0].item(), 3)
            self.assertEqual(ids[row, 1].item(), 6)
            self.assertEqual(ids[row, 2].item(), 1)
            self.assertEqual(ids[row, 3].item(), 5)

    def test_exact_tie_informational(self):
        # torch.topk tie order is implementation-defined per device;
        # documented as a known divergence risk, not asserted here.
        hidden = 128
        w = torch.zeros(8, hidden, device="cuda")
        x = torch.zeros(4, hidden, device="cuda")
        x[:, 0] = 1.0
