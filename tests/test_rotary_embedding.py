...
MODULES = load_operator_modules("rotary_embedding")
...
class TestRotaryEmbedding(unittest.TestCase):
    ...
    def _run(self, mod, T, H, D, dtype, interleaved=False):
        ...
        for name, mod in MODULES:
            for dtype in (torch.bfloat16, torch.float16, torch.float32):
                for T, H, D in [
                    ...
                    with self.subTest(
                        module=name, dtype=dtype, T=T, H=H, D=D
                    ):
                        self._run(mod, T, H, D, dtype)
    ...
    def test_fp32_tables(self):
        for name, mod in MODULES:
            with self.subTest(module=name):
                ...
                torch.manual_seed(1)
                x = torch.randn(
                    T, H, D, dtype=torch.bfloat16, device="cuda"
                )
                cos = torch.randn(
                    T, D // 2, dtype=torch.float32, device="cuda"
                )
                sin = torch.randn(
                    T, D // 2, dtype=torch.float32, device="cuda"
                )
    ...
    def test_noncontiguous(self):
        for name, mod in MODULES:
            with self.subTest(module=name):
                ...
                torch.manual_seed(2)
                base = torch.randn(
                    T, H, 2 * D, dtype=torch.float16, device="cuda"
                )
                x = base[:, :, :D]
                cos = torch.randn(
                    T, D // 2, dtype=torch.float16, device="cuda"
                )
                sin = torch.randn(
                    T, D // 2, dtype=torch.float16, device="cuda"
                )
    ...
    def test_input_invariance(self):
        for name, mod in MODULES:
            with self.subTest(module=name):
                x = torch.randn(
                    16, 8, 128, dtype=torch.bfloat16, device="cuda"
                )
                cos = torch.randn(
                    16, 64, dtype=torch.bfloat16, device="cuda"
                )
                sin = torch.randn(
                    16, 64, dtype=torch.bfloat16, device="cuda"
                )
    ...
    def test_empty(self):
        for name, mod in MODULES:
            with self.subTest(module=name):
                x = torch.empty(
                    0, 8, 64, dtype=torch.float16, device="cuda"
                )
                cos = torch.empty(
                    0, 32, dtype=torch.float16, device="cuda"
                )
                sin = torch.empty(
                    0, 32, dtype=torch.float16, device="cuda"
                )
