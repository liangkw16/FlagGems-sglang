...
METAX_MODULE = _load_module("fused_rmsnorm_metax_module", METAX_MODULE_PATH)
...
class DefaultLaunchPolicyTest(unittest.TestCase):
    def test_vendor_modules_use_official_default_launch(self):
        class FakeKernel:
            def __init__(self):
                self.kwargs = None
        ...
        for name, module in DEFAULT_LAUNCH_MODULES:
            with self.subTest(vendor=name):
                fake_kernel = FakeKernel()
                with mock.patch.object(
                    module, "_fused_rmsnorm_kernel", fake_kernel
                ):
                    module.fused_rmsnorm(
...
class FusedRmsnormTest(unittest.TestCase):
    def test_default_launch_matches_reference(self):
        for name, module in DEFAULT_LAUNCH_MODULES:
            for dtype, tolerance in (
                ...
                with self.subTest(vendor=name, dtype=dtype):
