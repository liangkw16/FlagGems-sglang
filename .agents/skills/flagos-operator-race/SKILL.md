...
开题第一小时还必须完成两件口径核定（2026-09-24 复盘教训，批 6
零 top1 的三个根因之二）：

- **额度日边界实测**：不要假设午夜重置。读 `status` 的 quota 字段
  并观察 `daily_seq` 是否跨 00:00 连续（批 6 实测为窗口内共享池、
  无午夜重置），据此推算剩余弹药再排作战计划；口径不明时按更保守
  的可用额度规划。
- **彩票榜首判据**：拉逐芯榜单（见「榜单逐芯情报通道」），凡榜首
  某芯读数 > 次优**同芯** 20x 即判窗口分，真实靶标=次优队读数而非
  榜首均值（批 6 实例：T78 燧原 2800x、T89 榜首 477837x、T85 华为
  21x、T91 燧原 6.5x；反例：T58 榜首 616.80 逐芯健康为真实结构）。
  对彩票榜首继续投同字节重掷是错靶。
...
浏览器/工单）；判断某题该不该打、打哪根轴前，先走「榜单逐芯情报通道」
（按题 leaderboard 端点返回**每支队伍的逐芯 speedup**，只读不耗额度）
——`operator-tasks` 的单一均值不足以判断榜差性质，据此得出的
"慢窗不可破译"结论多为错判。芯片硬事实坑表与 GQA 共享 dot 等结构资产见
同文档「平台评测踩坑硬事实」一节。“先不要提交平台”
