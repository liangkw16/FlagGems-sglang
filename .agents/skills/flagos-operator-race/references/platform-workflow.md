...
正式判定遵循仓库 `docs/competition/README.md` 的评分规则：正确性合格且每个支持
芯片的加速比均 `>=0.1` 才能排名；总分是全部芯片加速比的算术平均，同分较早提交
优先。部分完成、KernelGen/GPU 开发验证均不能代替本次 ZIP 的平台终态。平台自动
有效与专家评审后的荣誉资格分开记录；单题“攻克/攻占”不等于专项奖项或奖金。
辅助 MCP 计数缺失不新增提交前置条件；独立 release 回执及下述门禁仍须全部满足。
...
| 燧原 GCU 拒绝 i64 是**签名级**限制，不是"i64 数据流"全禁（源码已核实） | 批5 三题 `make_gcuir → Pipeline run failed`；TTIR 差分+逐轮平台实证（T59–T61 七轮）：i64 向量数据 load（clamp 恰只 int64 case 挂）、向量 load 值 extsi→i64 进 store 寻址（src2dst e1/e3）、多分支/字面量值 store（clamp e2–e7）均编译失败或写错地址。**报错文本订正**：`Pipeline run failed: PassManager execution failed` 是 Triton 后端 `make_gcuir` 的外层包装，按该串在 enflame 上游检索不到任何 issue/PR；上游真实文本是 `64-bit data type not supported on GCU300!` | `gcu64-type-verifier`（`third_party/enflame/triton_gcu/triton_gcu300/lib/Transforms/GCUSupportVerifier.cpp`）**只遍历 FuncOp 签名输入**，只判两项：64 位 pointee 指针（`!tt.ptr<i64>`）与裸 64 位标量参数（`i64`/`f64`）——**函数体内用 i64 做寻址算术不受该 pass 影响**（gcu300 默认挂 verifier，gcu400 需 `ENABLE_I64_CHECK`）。故处置收窄为**签名级**：i64 张量在 wrapper 以 `view(torch.int32)` 传入、标量一律 i32 域。i64 只用于寻址算术（arange/标量 extsi，kv_indices 121x 实证）；另一路是 T49 precomputed-pos（torch 预计算判定张量，kernel 纯乘法掩码）；scatter store 用 `src * stride` 纯 i32（decode_attention gather 镜像）；单 gather+单掩码 store+wrapper clone/零预填是 GCU 最稳形态（T59 27.82x）。对照：FlagGems `#5344`（closed 未合并，明确 "GCU300's make_gcuir PassManager rejects 64-bit dtypes"）、`libtriton_jit#41`（厂商确认 gcu300 无 int64 硬件支持、建议 int32）、FlagGems `#3471`（只在 **gcu400** 开 int64）。**同类路线勿想当然**：`tl.atomic_xchg` 缺证据，FlagTree `#1019` 已确认 XPU 不支持 `tl.atomic_cas` |
...
预注册晋级门（全部芯片的平均收益假设 + 最弱芯 0.1 门槛风险）与 stop gate。
在相同源码、harness 和环境上两次同指纹失败时，停止重复该候选，定位根因；
不据此否决整个算法方向。修复源码或改变有证据支持的假设后可进入新候选。
单芯显著回退先检查正确性、噪声及是否触及门槛；只要全部芯片仍达标且平均分
稳定提高，就可以晋级，不要求每芯都提高。部分芯片结果缺失时仅记录预计收益和
未知范围，不拿部分均值冒充全部芯片平均分；MCP/代理计时口径未对齐时不用作硬阈值。
