...
def decode_attention(q, k_buffer, v_buffer, kv_indptr, kv_indices, sm_scale):
    ...
    block_length_cap = 64 if kv_indices.numel() > batch_size * 32 else 32
    block_length = max(
        8, min(block_length_cap, 8192 // max(block_d, block_dv))
    )
    use_eight_warps = block_length == 64 and (
        kv_indptr.dtype == torch.int64
        or kv_indices.dtype == torch.int64
        or (q.dtype == torch.bfloat16 and block_d == 128 and block_dv == 128)
    )
    _decode_attention_kernel[(batch_size * query_heads,)](
        ...
        BLOCK_D=block_d,
        BLOCK_DV=block_dv,
        num_warps=8 if use_eight_warps else 4,
        num_stages=1,
    )
    return output
