...
def _context_attention_kernel(
    ...
    while query_block * BLOCK_M < sequence_length:
        ...
        for key_start in range(0, key_end, BLOCK_N):
            ...
            scores = tl.dot(query, keys, input_precision=DOT_PRECISION) * (
                sm_scale * log2e
            )
            ...
            accumulator = accumulator * correction[:, None] + tl.dot(
                probabilities, values, input_precision=DOT_PRECISION
            )
...
def context_attention(
    ...
    dot_precision = "tf32x3" if head_dim <= 128 else "ieee"
