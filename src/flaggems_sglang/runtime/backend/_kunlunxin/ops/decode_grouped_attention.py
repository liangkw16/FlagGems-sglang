...
def _decode_grouped_attention_kernel(
    ...
    dim_mask = dim < qk_dim
    safe_dim = tl.minimum(dim, qk_dim - 1)
    query = tl.load(
        q_ptr
        + batch * q_stride_batch
        + query_head * q_stride_head
        + safe_dim * q_stride_dim
    ).to(tl.float32)
    query = tl.where(dim_mask, query, 0.0)

    value_offset = value_tile * BLOCK_DV + tl.arange(0, BLOCK_DV)
    value_mask = value_offset < value_dim
    safe_value_offset = tl.minimum(value_offset, value_dim - 1)
    maximum = float("-inf")
    denominator = 0.0
    ...
    for position in range(0, sequence_length):
        ...
        key = tl.load(
            ...
            + safe_dim * k_stride_dim
        ).to(tl.float32)
        key = tl.where(dim_mask, key, 0.0)
        ...
        value = tl.load(
            ...
            + safe_value_offset * v_stride_dim
        ).to(tl.float32)
...
def decode_grouped_attention(
    ...
    output_shape = (batch_size, query_heads, value_dim)
    if batch_size == 0 or query_heads == 0 or value_dim == 0:
        return torch.empty(output_shape, dtype=torch.float32, device=q.device)
    ...
    padded_value_dim = value_tiles * _BLOCK_DV
    output_storage = torch.empty(
        (batch_size, query_heads, padded_value_dim),
        dtype=torch.float32,
        device=q.device,
    )
    ...
    for program_start in range(0, total_programs, _MAX_GRID_PROGRAMS):
        ...
        _decode_grouped_attention_kernel[(program_count,)](
            q,
            k_buffer,
            v_buffer,
            routed_indptr,
            ...
            output_storage.stride(0),
            output_storage.stride(1),
            output_storage.stride(2),
            program_start,
            value_tiles,
            BLOCK_D=block_d,
            BLOCK_DV=_BLOCK_DV,
    ...
    return output_storage[..., :value_dim]
