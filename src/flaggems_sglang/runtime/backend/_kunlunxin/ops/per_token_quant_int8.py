...
def _per_token_quant_int8_kernel(
    ...
    # kunlunxin vendor (e2): two-pass column-blocked scan at the T21-proven
    # kunlun reduction BLOCK 1024 instead of one next_pow2(N)-wide tile
    # (BLOCK is the only consistently effective axis on kunlun); chunk-wise
    # running max is order-invariant, all math (fp32 amax, div_rn,
    # truncating int8 cast) is identical to the generic kernel
    pid = tl.program_id(0)
