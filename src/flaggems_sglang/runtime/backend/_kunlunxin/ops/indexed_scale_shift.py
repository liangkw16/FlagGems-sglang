...
@triton.jit
def _round_scale(
    scale, one_plus, cs0, HDIM: tl.constexpr, BLOCK: tl.constexpr
):
    variant = tl.program_id(0)
    offs = tl.program_id(1) * BLOCK + tl.arange(0, BLOCK)
    m = offs < HDIM
    sc = tl.load(scale + variant * cs0 + offs, m, other=0).to(tl.float32)
    rounded = (1.0 + sc).to(tl.bfloat16, fp_downcast_rounding="rtne")
    tl.store(one_plus + variant * HDIM + offs, rounded, m)
...
def _scale_rows(
    ...
    for row in range(tl.program_id(0), rows, tl.num_programs(0)):
        ...
        for h0 in tl.static_range(0, HDIM, BLOCK):
            ...
            factor = tl.load(one_plus + idx * HDIM + offs, m, other=0).to(
                tl.float32
            )
...
def indexed_scale_shift(x, shift, scale, indices):
    ...
    if rows and hdim:
        one_plus = torch.empty(
            (num_variants, hdim), dtype=x.dtype, device=x.device
        )
        block = min(1024, triton.next_power_of_2(hdim))
        _round_scale[(num_variants, triton.cdiv(hdim, block))](
            scale, one_plus, scale.stride(0), HDIM=hdim, BLOCK=block
        )
        grid = (min(rows, 2048),)
        _scale_rows[grid](
            x,
