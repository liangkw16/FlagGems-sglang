...
# Kunlunxin vendor: the S0 run died on this chip with
# OutOfResources: uni_sram - the [C, S] tile needs this stack's
# smaller SRAM budget. Same kernel, tile lanes capped at 2048
# (and BLOCK_S floors at 32 per the FlagGems kunlunxin notes on
# tiny-tile miscompiles).
...
def group_norm_silu(x, weight, bias, num_groups, eps):
    ...
    if n_groups_total and spatial:
        ...
        block_s = min(
            triton.next_power_of_2(spatial), max(32, 2048 // block_c)
        )
