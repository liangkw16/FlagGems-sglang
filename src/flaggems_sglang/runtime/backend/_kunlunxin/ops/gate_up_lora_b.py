...
# Kunlunxin vendor: mirrors the Enflame-safe structure (1D grid fold,
# no runtime early-return branches, rank-0/empty segments contribute zero
# via scaling) - runtime branches are the remaining suspect for the
# kunlun crash fingerprint on the generic 3D-grid variant.
...
def _gate_up_lora_b_kernel(
    ...
    n_tiles = matrix_blocks * 2 * n_segments
    n_programs = tl.num_programs(0)
    for tile in tl.range(tl.program_id(0), n_tiles, n_programs):
        slice_id = tile % 2
        tile_rest = tile // 2
        batch_id = tile_rest % n_segments
        matrix_pid = tile_rest // n_segments
        ...
        lora_rank = tl.load(lora_ranks_ptr + weight_index)
        scaling = tl.load(scalings_ptr + weight_index).to(tl.float32)
        factor = tl.where(lora_rank == 0, 0.0, scaling)
        ...
        tl.store(output_ptrs, base + accumulator * factor, mask=mask)
...
def gate_up_lora_b(x, gate_up_lora_b, batch_info, output_dim, base_output):
    ...
    if max_len is None:
        max_len = int(
            (batch_info.seg_indptr[1:] - batch_info.seg_indptr[:-1])
            .max()
            .item()
        )
    if max_len == 0:
    ...
    n_tiles = matrix_blocks * 2 * bs
    permutation = batch_info.permutation
    _gate_up_lora_b_kernel[(min(n_tiles, 65535),)](
        x,
        gate_up_lora_b,
        output,
