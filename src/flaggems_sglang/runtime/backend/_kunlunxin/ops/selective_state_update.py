...
def _selective_state_update_kernel(
    ...
    # Direct 3D grid avoids the XPU backend's flat-grid div/mod loop.
    tile_in_head = tl.program_id(0)
...
def selective_state_update(
    ...
    for batch_start in range(0, batch, batch_chunk):
        segment_batch = min(batch_chunk, batch - batch_start)
        grid = (tiles_per_head, segment_batch, nheads)
