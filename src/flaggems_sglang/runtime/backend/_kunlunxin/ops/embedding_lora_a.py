...
def _embedding_lora_a_kernel(
    ...
    for token in range(token_offset, max_len, token_total):
        if token < segment_length and rank != 0:
            token_id = tl.load(
                input_ids + (segment_start + token) * input_stride
            )
            for rank_block in range(num_rank_blocks):
                rank_offsets = rank_block * BLOCK_RANK + tl.arange(
                    0, BLOCK_RANK
                )
                ...
                tl.store(
                    output
                    + (segment_start + token) * output_stride_token
...
def embedding_lora_a(
    ...
    token_cap = max(1, 65535 // batch_info.bs)
    grid = (min(batch_info.max_len, token_cap), batch_info.bs)
