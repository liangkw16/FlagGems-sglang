...
# Enflame vendor for create_flashmla_kv_indices: the T63-proven GCU
# recipe translated to page space - launch held at the 24-SIP physical
# width (splits capped, grid.x = 24 // splits), BLOCK_P 512 pages per
# tile, num_stages >= 3 for the pingpong pipeline, and no num_warps pin
# (any launch pin degraded the family ~50% on T62 e11/e12/e13).
...
def create_flashmla_kv_indices(
    ...
    if not (batch and width and out.numel()):
        ...
        return out
    # GCU recipe: cap the launch at the 24-SIP width; rows grid-stride
    # beyond grid.x and page tiles stride across <= 4 splits.
    splits = min(max(1, triton.cdiv(width, 512)), 4)
