...
@triton.jit
def _mamba_layernorm_gated_multirow_kernel(
    x_ptr,
    weight_ptr,
    bias_ptr,
    z_ptr,
    ...
    columns = tl.arange(0, BLOCK_SIZE)
    flat_columns = group * GROUP_SIZE + columns
    mask = (row_offsets[:, None] < rows) & (columns[None, :] < GROUP_SIZE)

    x = tl.load(
        x_ptr
        + row_offsets[:, None] * stride_x_row
        + flat_columns[None, :] * stride_x_col,
        mask=mask,
        other=0.0,
    ).to(tl.float32)
    if HAS_Z and not NORM_BEFORE_GATE:
        z = tl.load(
            z_ptr
            + row_offsets[:, None] * stride_z_row
            + flat_columns[None, :] * stride_z_col,
            mask=mask,
            other=0.0,
        ).to(tl.float32)
        x = x * z * tl.sigmoid(z)
    ...
    variance = tl.sum(centered * centered, axis=1) / GROUP_SIZE
...
def mamba_layernorm_gated(
    ...
    if block_size == 512 and group_size < 512:
        num_warps = 2
    rows_per_block = 1
    if group_size <= 128 and rows * hidden_size // group_size >= 4096:
        rows_per_block = min(
            triton.next_power_of_2(rows), triton.cdiv(1024, block_size)
        )
    ...
    if rows_per_block == 1:
        _mamba_layernorm_gated_kernel[(rows, group_count)](
            *arguments, *strides, **launch_options
        )
    else:
        _mamba_layernorm_gated_multirow_kernel[
            (triton.cdiv(rows, rows_per_block), group_count)
        ](
            *arguments,
            rows,
            *strides,
