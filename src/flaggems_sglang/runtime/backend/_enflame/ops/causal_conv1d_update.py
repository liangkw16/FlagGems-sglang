...
# Enflame vendor: exact 2D grid, no grid-stride runtime loop and no
# int64 row-base casts - S0 scored 0.0265x, below the 0.1 threshold.
...
def _causal_conv1d_update_kernel(
    ...
    for t in range(0, seqlen):
        ...
        for k in tl.static_range(WIDTH):
            ...
            s_v = tl.load(
                s_base + offs_d * st_sd + safe_s * st_sl,
                mask=dmask & from_state,
                other=0.0,
            ).to(tl.float32)
            x_v = tl.load(
                x_base + offs_d * x_sd + safe_x * x_ss,
