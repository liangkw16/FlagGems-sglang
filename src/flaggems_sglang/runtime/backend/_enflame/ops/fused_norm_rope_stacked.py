...
# Enflame vendor: three-kernel split mirroring platform-proven forms
# from this backend. (A) RMSNorm row kernel in the T51-proven shape
# (one program per (t, l, h), 1D [BLOCK_D], tl.rsqrt, no grid-stride
# loop) writing an fp32 intermediate; (B) pair RoPE reading the fp32
# intermediate and casting once to the output dtype; (C) a plain
# contiguous row copy for V. E5 lesson applied: no explicit
# num_warps/num_stages so the GCU backend picks its official default
# launch (T19 E5 / T51 E5 platform-proven); the fused e1/e2 forms hit
# PassManager crashes on both prior structures.
