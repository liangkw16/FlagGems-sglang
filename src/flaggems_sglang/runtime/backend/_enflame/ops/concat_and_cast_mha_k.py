...
# Enflame vendor for concat_and_cast_mha_k: the T73/T75 streaming
# recipe - launch held at the 24-SIP physical width (the task
# grid-stride loop already lives in the kernel body), num_stages >= 3
# for the pingpong pipeline, and no num_warps pin (every launch pin
# degraded this op family on T62 e11/e12/e13). Kernel bytes are the
# generic form.
...
def concat_and_cast_mha_k(k, k_nope, k_rope):
    ...
    if out.numel():
        ...
        tasks = tokens * groups
        _concat_and_cast_mha_k[(min(tasks, 24),)](
            k_nope,
            k_rope,
            out,
            tasks,
