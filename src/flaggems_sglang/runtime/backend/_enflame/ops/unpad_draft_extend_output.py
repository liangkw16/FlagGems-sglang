...
# Enflame vendor for unpad_draft_extend_output: the GCU streaming
# recipe on the staged-gather form - grid capped at the 24-SIP width
# with the row grid-stride already in the body, num_stages 3, no warps
# pin (our enflame reads 3.5 vs the leader band).
...
def unpad_draft_extend_output(raw_out, cu_seqlens_q, seq_lens_q, sum_seq_lens_q):
    ...
    if bs and token_per_batch and out.numel():
        ...
        row_span = heads * dim
        _copy_rows[(min(total, 24), min(max(1, triton.cdiv(row_span, 1024)), 255))](
            raw_out,
            src_row,
            out,
            total,
