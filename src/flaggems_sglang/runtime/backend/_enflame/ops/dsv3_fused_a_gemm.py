...
# Enflame vendor: GCU recipe on the generic GEMM - launches unpinned
# from num_warps (unpinned median 2.077x vs pinned 0.732x on this
# stack) and capped at the 24-SIP physical width; the kernel bodies
# already stride in-kernel. BLOCK/tile geometry unchanged.
...
def _dsv3_fused_a_gemm_baseline(mat_a, mat_b):
    ...
    if m and n and k:
        _dsv3_fused_a_gemm[(min(triton.cdiv(n, 64), 24),)](
            mat_a,
            mat_b,
            out,
            m,
...
def _dsv3_reduce_splitk(partials, out, size, BLOCK: tl.constexpr):
    for tile in range(tl.program_id(0), tl.cdiv(size, BLOCK), tl.num_programs(0)):
...
def dsv3_fused_a_gemm(mat_a, mat_b):
    ...
    _dsv3_fused_a_gemm_splitk[(min(triton.cdiv(n, 64) * 4, 24),)](
        mat_a,
        mat_b,
        partials,
        m,
    ...
    _dsv3_reduce_splitk[(min(triton.cdiv(m * n, 256), 24),)](
        partials, out, m * n, BLOCK=256
    )
    return out
