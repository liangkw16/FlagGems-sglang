...
# Iluvatar (tianshu) vendor: this chip lost 12% when the generic moved to
# BLOCK=4096 in e1 (4.769 -> 4.191) while enflame gained 71%, so the
# vendor freezes the s0 BLOCK=1024 for this chip.
...
def sigmoid_gate_mul(x, gate):
    ...
    if n:
        _sigmoid_gate_mul[(min(triton.cdiv(n, _BLOCK), _MAX_GRID),)](
