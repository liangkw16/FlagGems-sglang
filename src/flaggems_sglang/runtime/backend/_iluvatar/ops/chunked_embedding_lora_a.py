...
@triton.jit
def _cela_gather_kernel_iluvatar(
    input_ids,
    weights,
    output,
    permutation,
    ...
    # Iluvatar variant (T46 plan candidate 1): i32 pre-routed
    # straight-line gather - searchsorted route prep replaces the
    # per-token binary search (~log2(B) serialized scalar loads per
    # token) that the generic kernel pays. Metax proved +26% with this
    # exact structure; tianshu runs the same generic binary search on
    # a much larger score base (9.2x+ chips read 21-32x).
    pid = tl.program_id(0)
...
def chunked_embedding_lora_a(input_ids, weights, batch_info, vocab_size):
    ...
    input_ids32 = (
        input_ids if input_ids.dtype == torch.int32 else input_ids.to(torch.int32)
    )
    ...
    _cela_gather_kernel_iluvatar[grid](
        input_ids32,
        weights,
        output,
        permutation32,
