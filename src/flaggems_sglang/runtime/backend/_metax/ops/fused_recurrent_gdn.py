...
# MetaX E9 candidate: fixed eight-accumulator pairwise order (chains over k %
# 8, merge ((a0+a1)+(a2+a3))+((a4+a5)+(a6+a7))).
...
def _fused_recurrent_gdn_k64_kernel(
    ...
    for timestep in range(0, sequence_length):
        ...
        prediction_6 = 0.0
        prediction_7 = 0.0
        for key_offset in tl.static_range(0, 64, 8):
            lane_0 = tl.load(state_ptr + state_base + key_offset) * decay
            lane_1 = tl.load(state_ptr + state_base + key_offset + 1) * decay
            lane_2 = tl.load(state_ptr + state_base + key_offset + 2) * decay
            lane_3 = tl.load(state_ptr + state_base + key_offset + 3) * decay
            lane_4 = tl.load(state_ptr + state_base + key_offset + 4) * decay
            lane_5 = tl.load(state_ptr + state_base + key_offset + 5) * decay
            lane_6 = tl.load(state_ptr + state_base + key_offset + 6) * decay
            lane_7 = tl.load(state_ptr + state_base + key_offset + 7) * decay
            tl.store(state_ptr + state_base + key_offset, lane_0)
            tl.store(state_ptr + state_base + key_offset + 1, lane_1)
            tl.store(state_ptr + state_base + key_offset + 2, lane_2)
            tl.store(state_ptr + state_base + key_offset + 3, lane_3)
            tl.store(state_ptr + state_base + key_offset + 4, lane_4)
            tl.store(state_ptr + state_base + key_offset + 5, lane_5)
            tl.store(state_ptr + state_base + key_offset + 6, lane_6)
            tl.store(state_ptr + state_base + key_offset + 7, lane_7)
            ...
            key_4 = tl.load(
                k_ptr + key_base + (key_offset + 4) * stride_k_dim
            ).to(tl.float32)
            key_5 = tl.load(
                k_ptr + key_base + (key_offset + 5) * stride_k_dim
            ).to(tl.float32)
            key_6 = tl.load(
                k_ptr + key_base + (key_offset + 6) * stride_k_dim
            ).to(tl.float32)
            key_7 = tl.load(
                k_ptr + key_base + (key_offset + 7) * stride_k_dim
            ).to(tl.float32)
            prediction_0 = tl.fma(lane_0, key_0, prediction_0)
            prediction_1 = tl.fma(lane_1, key_1, prediction_1)
            prediction_2 = tl.fma(lane_2, key_2, prediction_2)
            prediction_3 = tl.fma(lane_3, key_3, prediction_3)
            prediction_4 = tl.fma(lane_4, key_4, prediction_4)
            prediction_5 = tl.fma(lane_5, key_5, prediction_5)
            prediction_6 = tl.fma(lane_6, key_6, prediction_6)
            prediction_7 = tl.fma(lane_7, key_7, prediction_7)
        prediction = (
        ...
        for key_offset in tl.static_range(0, 64, 8):
            ...
            tl.store(
                outer_ptr + state_base + key_offset + 2,
                correction * key_2,
            )
            tl.store(
                outer_ptr + state_base + key_offset + 3,
                correction * key_3,
            )
            tl.store(
                outer_ptr + state_base + key_offset + 4,
                correction * key_4,
            )
            tl.store(
                outer_ptr + state_base + key_offset + 5,
                correction * key_5,
            )
            tl.store(
                outer_ptr + state_base + key_offset + 6,
                correction * key_6,
            )
            tl.store(
                outer_ptr + state_base + key_offset + 7,
                correction * key_7,
            )
        for key_offset in tl.static_range(0, 64, 8):
            ...
            lane_4 = tl.load(
                state_ptr + state_base + key_offset + 4
            ) + tl.load(outer_ptr + state_base + key_offset + 4)
            lane_5 = tl.load(
                state_ptr + state_base + key_offset + 5
            ) + tl.load(outer_ptr + state_base + key_offset + 5)
            lane_6 = tl.load(
                state_ptr + state_base + key_offset + 6
            ) + tl.load(outer_ptr + state_base + key_offset + 6)
            lane_7 = tl.load(
                state_ptr + state_base + key_offset + 7
            ) + tl.load(outer_ptr + state_base + key_offset + 7)
            tl.store(state_ptr + state_base + key_offset, lane_0)
            tl.store(state_ptr + state_base + key_offset + 1, lane_1)
            tl.store(state_ptr + state_base + key_offset + 2, lane_2)
            tl.store(state_ptr + state_base + key_offset + 3, lane_3)
            ...
            query_4 = (
                tl.load(
                    q_ptr + query_base + (key_offset + 4) * stride_q_dim
                ).to(tl.float32)
                * scale
            )
            query_5 = (
                tl.load(
                    q_ptr + query_base + (key_offset + 5) * stride_q_dim
                ).to(tl.float32)
                * scale
            )
            query_6 = (
                tl.load(
                    q_ptr + query_base + (key_offset + 6) * stride_q_dim
                ).to(tl.float32)
                * scale
            )
            query_7 = (
                tl.load(
                    q_ptr + query_base + (key_offset + 7) * stride_q_dim
                ).to(tl.float32)
                * scale
            )
            result_0 = tl.fma(lane_0, query_0, result_0)
            result_1 = tl.fma(lane_1, query_1, result_1)
            result_2 = tl.fma(lane_2, query_2, result_2)
            result_3 = tl.fma(lane_3, query_3, result_3)
            result_4 = tl.fma(lane_4, query_4, result_4)
            result_5 = tl.fma(lane_5, query_5, result_5)
            result_6 = tl.fma(lane_6, query_6, result_6)
            result_7 = tl.fma(lane_7, query_7, result_7)
