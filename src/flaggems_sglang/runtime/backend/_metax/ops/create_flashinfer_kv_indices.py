...
# Metax (muxi) vendor: byte-frozen copy of the round-1 generic (BLOCK=512,
# splits capped at 32) that this chip passed on the platform at its best
# reading; the round-2 generic's BLOCK=256 / 512-program split lifted the
# large GPUs but regressed the narrow-bandwidth chips (enflame 21.7 ->
# 9.0, kunlun 2.88 -> 2.45, muxi 60.3 -> 55.1), so those chips keep the
# proven bytes while the big GPUs take the new shape.
