...
def _cela_gather_kernel_metax(
    ...
    # Metax variant: same i32 pre-routed straight-line gather as the
    # enflame vendor - searchsorted route prep replaces the per-token
    # binary search (~10 scalar loads per token), which the generic
    # kernel pays on every chip without a vendor file.
    pid = tl.program_id(0)
...
def chunked_embedding_lora_a(input_ids, weights, batch_info, vocab_size):
    ...
    _cela_gather_kernel_metax[grid](
        input_ids32,
        weights,
        output,
        permutation32,
