...
def _act_and_mul_flat_kernel(
    ...
    # metax vendor: flat output-element grid-stride over BLOCK_SIZE
    # lanes (flat full-coverage large BLOCK, +65.68% on T39 / +11% on T29); activation math identical to the generic
    # kernel including the input-dtype multiply after the fp32
    # activation is cast back.
    pid = tl.program_id(0)
    ...
    for start in range(pid * BLOCK_SIZE, n_elements, step):
        ...
        gate = tl.load(
            x_ptr + input_base, mask=mask, other=0.0
        ).to(tl.float32)
        up = tl.load(
            x_ptr + input_base + half_width, mask=mask, other=0.0
        ).to(tl.float32)
        if HAS_LIMIT:
