...
def moe_sum_reduce(input, routed_scaling_factor):
    ...
    use_qwen_launch = (
        input.is_contiguous() and top_k == 8 and hidden_dim in (2048, 4096)
    )
    block_size = 1024 if use_qwen_launch else 256
    launch_kwargs = {"num_warps": 8 if use_qwen_launch else 4}
    if not use_qwen_launch:
