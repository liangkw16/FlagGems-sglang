...
def _chunk_state_kernel(
    ...
    n_tiles_axis = tl.cdiv(dstate, BLOCK_N)
    m_tiles_axis = tl.cdiv(headdim, BLOCK_M)
    tiles_per_head = m_tiles_axis * n_tiles_axis

    for logical_id in range(program_id, total_programs, grid_size):
        head_id = logical_id // (batch_chunks * tiles_per_head)
        remainder = logical_id - head_id * (batch_chunks * tiles_per_head)
        batch_chunk_id = remainder // tiles_per_head
        tile_id = remainder - batch_chunk_id * tiles_per_head
        batch_id = batch_chunk_id // nchunks
        chunk_id = batch_chunk_id - batch_id * nchunks
        group_id = head_id // head_group_ratio
        m_tile = tile_id // n_tiles_axis
        n_tile = tile_id - m_tile * n_tiles_axis
        ...
        for k_block in range(0, tl.cdiv(chunk_size, BLOCK_K)):
            ...
            if LOWPREC:
                B = (B.to(tl.float32) * scale[:, None]).to(
...
def chunk_state(B, x, dt, dA_cumsum):
    ...
    lowprec = (
        ...
        and headdim in (64, 128)
        and dstate in (64, 128)
        and chunk_size in (64, 128, 256)
    )
    if lowprec:
        block_m = 64
        block_n = 64
