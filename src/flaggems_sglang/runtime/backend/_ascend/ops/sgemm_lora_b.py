...
def _sgemm_lora_b_kernel(
    ...
    tiles_per_batch = tl.cdiv(max_len, BLOCK_S) * num_output_blocks
    for logical_id in range(program_id, total_programs, grid_size):
        batch_id = logical_id // tiles_per_batch
        matrix_pid = logical_id - batch_id * tiles_per_batch
        ...
        if token_block * BLOCK_S < segment_length and rank != 0:
...
def sgemm_lora_b(x, weights, batch_info, base_output):
    ...
    tiles_per_batch = triton.cdiv(batch_info.max_len, block_s) * output_blocks
    total_programs = tiles_per_batch * batch_info.bs
