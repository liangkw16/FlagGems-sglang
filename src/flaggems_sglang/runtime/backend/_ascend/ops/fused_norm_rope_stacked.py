...
# Ascend vendor: three-kernel split mirroring platform-proven forms
# from this backend. (A) RMSNorm row kernel in the T51-proven shape
# (one program per (t, l, h), 1D [BLOCK_D], `x * 1.0 / tl.sqrt(...)`
# exactly as the passing fla_layernorm_gated vendor) writing an fp32
# intermediate; (B) pair RoPE reading the fp32 intermediate and casting
# once to the output dtype; (C) a plain contiguous row copy for V.
# Every mask is a single 1D predicate: no 2D tiles (2D-broadcast NaN
# family), no compound masks, and the fused e1/e2 forms mismatched the
# largest H=2/D=1024 case here.
