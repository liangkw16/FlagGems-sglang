...
def _decode_grouped_attention_kernel(
    ...
    for program_id in range(worker_id, total_programs, worker_count):
...
def decode_grouped_attention(
    ...
    total_programs = batch_size * query_heads
    worker_count = min(
        total_programs,
        _get_num_vector_cores(q.device.index),
    )
    _decode_grouped_attention_kernel[(worker_count,)](
        q,
        k_buffer,
        v_buffer,
        routed_indptr,
