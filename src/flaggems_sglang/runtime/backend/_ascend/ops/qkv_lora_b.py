...
def _qkv_lora_b_kernel(
    ...
    tiles_per_slice = tl.cdiv(max_len, BLOCK_S) * num_output_blocks
    for logical_id in range(program_id, total_programs, grid_size):
        batch_id = logical_id // (n_slices * tiles_per_slice)
        remainder = logical_id - batch_id * (n_slices * tiles_per_slice)
        slice_id = remainder // tiles_per_slice
        matrix_pid = remainder - slice_id * tiles_per_slice
        ...
        output_start = tl.load(
            output_offset_ptr + slice_id * output_offset_stride
        )
        output_end = tl.load(
        ...
        if (
            token_block * BLOCK_S < segment_length
            and output_block * BLOCK_N < output_size
            and tl.load(lora_ranks_ptr + weight_index * lora_ranks_stride) != 0
        ):
            ...
            for k_start in range(0, RANK, BLOCK_K):
                ...
                weights = tl.load(
                    ...
                    + (output_start + output_offsets[None, :])
                    * weight_stride_output
...
def qkv_lora_b(
    ...
    tiles_per_slice = triton.cdiv(batch_info.max_len, block_s) * output_blocks
    total_programs = tiles_per_slice * n_slices * batch_info.bs
