# Ascend vendor (e9 probe): generic bytes with a single launch pin -
# num_warps=8 (huawei reads 1.3 vs Albedo's 2.5 on this task; the
# PR-corpus's cheapest per-chip knob). Gate huawei >= 1.7.
...
def sigmoid_gate_mul(x, gate):
    ...
    if n:
        _sigmoid_gate_mul[(min(triton.cdiv(n, _BLOCK), 65535),)](
            x, gate, out, n, BLOCK=_BLOCK, num_warps=8
        )
    return out
