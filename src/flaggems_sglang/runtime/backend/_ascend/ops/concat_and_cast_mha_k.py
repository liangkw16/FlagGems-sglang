...
# Ascend vendor for concat_and_cast_mha_k: the T62 e12/e13-proven
# copy-family persistent launch (grid capped at num_vectorcore, kernel
# bytes = generic, task grid-stride already in the body) that lifted
# T62 huawei 0.284 -> 0.332 (+16.7% on e13).
...
def concat_and_cast_mha_k(k, k_nope, k_rope):
    ...
    if out.numel():
        ...
        tasks = tokens * groups
        _concat_and_cast_mha_k[(min(tasks, _worker_count(k)),)](
            k_nope,
            k_rope,
            out,
            tasks,
