...
# Ascend vendor for unpad_draft_extend_output: persistent launch capped
# at the Vector Core count (copy-family recipe; our huawei 58-69 vs
# the 100+ band on other chips).
...
def unpad_draft_extend_output(raw_out, cu_seqlens_q, seq_lens_q, sum_seq_lens_q):
    ...
    if bs and token_per_batch and out.numel():
        ...
        row_span = heads * dim
        _copy_rows[(min(total, _worker_count(out)), min(max(1, triton.cdiv(row_span, 1024)), 255))](
            raw_out,
            src_row,
            out,
            total,
