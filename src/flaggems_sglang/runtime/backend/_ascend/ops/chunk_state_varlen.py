...
def _chunk_state_varlen_kernel(
    ...
    m_tiles = tl.cdiv(headdim, BLOCK_M)
    tiles_per_head = m_tiles * n_tiles
    for logical_id in range(program_id, total_programs, grid_size):
        head = logical_id // (batch_count * tiles_per_head)
        remainder = logical_id - head * (batch_count * tiles_per_head)
        batch = remainder // tiles_per_head
        tile = remainder - batch * tiles_per_head
        tile_m = tile // n_tiles
        tile_n = tile % n_tiles
        ...
        for block_start in range(0, sequence_length, BLOCK_K):
            ...
            accumulator += tl.dot(
                x, B * scale[:, None], input_precision="ieee"
            )
...
def chunk_state_varlen(B, x, dt, dA_cumsum, cu_seqlens, chunk_states):
    ...
    total_programs = tiles_per_head * batch * nheads
