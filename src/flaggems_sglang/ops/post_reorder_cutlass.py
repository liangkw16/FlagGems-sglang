...
# CUTLASS-MoE epilogue: merge each token's routed expert outputs in
# fp32, skipping slots routed off-rank (topk_ids == num_local_experts),
# and fold the routed scaling factor into the store. One program per
# token with HDIM constexpr; slot loop is a static unroll (the T81
# mechanism), gathers carry the masked arithmetic guard.
...
def post_reorder_cutlass(
    ...
    if num_tokens and hidden_size:
        _post_reorder[(min(num_tokens, 2048),)](
            down_output,
            src2dst,
            topk_ids,
            topk_weights,
