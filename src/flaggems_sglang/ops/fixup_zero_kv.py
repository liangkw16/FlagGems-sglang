...
# decides everything on-device in one launch. E8 lands the fix in place
# - the task's own semantic ("zeroes their output rows in a single
# launch ... no host sync"): out/lse are the operator's outputs, only
# zero-KV segments are written (constants 0 / -inf) and untouched rows
# keep their bytes, so a batch with no zero-KV request costs one launch
# and nothing else. The returned tensors are the passed buffers, the
# same in-place-outparam shape the platform already accepted for the
# moe_align_block_size outputs since e12.
