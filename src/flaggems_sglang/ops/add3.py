...
# Adapted from SGLang 197832b kernels/ops/elementwise/add3.py: the
# double rounding bf16(bf16(a + b) + c) is the contract - each add
# computes in fp32 and rounds to bf16 exactly like the unfused torch
# pair, so fusing the loads must not collapse the intermediate round.
...
def add3(a, b, c):
    ...
    if numel:
        _add3[(min(triton.cdiv(numel, 1024), 4096),)](
