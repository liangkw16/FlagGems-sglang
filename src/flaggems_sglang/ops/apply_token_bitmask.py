...
def _apply_token_bitmask_kernel(
    ...
    batch = program_id // blocks_per_row
    block = program_id % blocks_per_row
    token = block * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
    token_mask = token < vocab_size

    logits = tl.load(
        logits_ptr + batch * logits_stride_batch + token * logits_stride_vocab,
        mask=token_mask,
        other=0.0,
    )
    packed = tl.load(
    ...
    output = tl.where(allowed, logits, -float("inf"))
    tl.store(
        output_ptr + batch * output_stride_batch + token * output_stride_vocab,
        output,
        mask=token_mask,
    )
