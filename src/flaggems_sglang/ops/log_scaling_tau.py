...
def _direct_launcher(compiled, grid0):
    # Prebound launcher for the already-compiled fast kernel: only a
    # stream query plus the C launcher call, skipping the per-call
    # Python binder of the standard JIT dispatch. This is the same
    # launch shape torch.compile's generated wrappers use. Builds only
    # when this Triton exposes the documented CompiledKernel API and no
    # launch hook is registered; the SAME Triton kernel executes either
    # way - there is no torch fallback.
...
def _make_fast_dispatch():
    ...
    def _fast(x, tau, out, rows, n_cols):
        ...
        if entry is None:
            ...
            # path must preserve); misaligned calls re-enter the
            # standard dispatch above.
            launchers[key] = _direct_launcher(ck, grid0) if aligned else False
