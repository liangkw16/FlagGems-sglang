...
## 2026-08-27 08:33 CST 只读终态汇总与 E13 预注册（未新增提交）

四笔提交（5215/E9、5218/E10、5221/E11、5223/E12）除燧原外全部终态；
本节仅只读 `status` 汇总与下一变量预注册，未运行 preflight、未消耗额度。

| 芯片 | 5215 (E9) | 5218 (E10) | 5221 (E11) | 5223 (E12) |
| --- | ---: | ---: | ---: | ---: |
| 国际 A（vendor） | 5.4985x | 5.369x | 5.5315x | 5.359x |
| 海光 | 3 | 15/16 | 3 | 3 |
| 国际 B | 567/34 | 402/26 | 402/26 | 844/38 |
| 天数 | 893/4 | 893/4 | 893/4 | 893/4 |
| 沐曦 | 131052/16 | 131052/16 | 131052/16 | 131052/16 |
| 华为 | 531/6 | 531/6 | 531/6 | 531/6 |
| 燧原 | 回调未达 | 回调未达 | 回调未达 | 回调未达 |
| 昆仑芯 | 终态失败 | 终态失败 | 终态失败 | 终态失败 |

- 燧原四笔跨约 7 小时无回调，为平台侧评测停摆；换字节也无法读到结果。
- 昆仑芯四笔均终态失败且无 case 详情，与既往 reference 侧 1830s/1800s
  崩溃指纹一致，与提交字节无关。
- 额度自 01:50 的 16/30 降至 **3/30**（并行会话消耗 13 次）；截止
  19:59:59。燧原+昆仑两个平台侧阻塞使今日 8/8 攻克不可达（题目仍
  `pending_challenge`）；现实目标改为终投一次"全赢家组装"最大化单次通过芯数。

### 对昨夜队列的关键修正：串行族从未在平坦三芯/燧原上试过

E9 给天数/沐曦/华为/燧原分发的是 R4 交叉/R8 成对/相邻树/evenodd，
串行 FMA 只发到了海光与国际 B——而它恰是唯一在两芯同时大幅命中的族
（海光 740→3、国际 B 844→402）。"对所有树形平坦、唯串行塌缩"是
GEMV 型后端（瘦形状逐输出线程串行累加）的典型指纹；NVIDIA 的 R4 交叉
只是 cuBLAS 的特例选择，不能外推其他厂商的 GEMV 形态。

### E13 预注册（一发六探针，消耗 1 额度，待执行）

- `_iluvatar`/`_metax`/`_ascend`/`_enflame` → 串行 FMA（最高优先级；
  燧原即使不回调也不占额外额度）；
- `_hygon` → 串行树冻结，仅把 libdevice exp 换 `tl.exp`：3 元素残差属
  "极稀有 1-ulp"量级，GEMV 反向扫描概率低，exp 实现是头号嫌疑
  （E4 在 NVIDIA 实证 exp 选择决定 bitwise 一致性）；
- `_amd` → 反向串行（402 之后族谱最便宜的下一个点）；
- generic 与 `_nvidia` 字节冻结。

判读规则：平坦三芯若塌缩至个位数 → E14 精修 + E15 全赢家组装终投；
完全不动 → 其阻塞在 exp/精度模式层，剩余额度转保海光/国际 B 清零。
无论走哪条路径，保底预留 1 次额度给终投。

### 剩余不再尝试清单

平坦三芯的更多树形变体（串行之前一切树形已证明无效）、`tl.dot`
input_precision 路线（E3 代理证伪且更慢）、昆仑定向改动（崩溃在
reference 侧）、NVIDIA 文件任何改动（五连过）、非 K64/L2 路径
（隐藏 case 全部走 K64 分支）。
