...
## 2026-09-11 E7（guard 快路径）：燧原数值仍 100% 错，七轮止损确认

- E7（daily_seq 26，09:09，commit `26eb3ac`）：wrapper 值域 guard
  （一次归约 + host sync，双路径均为 Triton）——值全部落于 [0, 2^31) 时走
  GCU 已数值验证的 i32 内核；否则回退 64 位词内核。燧原仍 100% 错，
  **相对差指纹与 E5/E6 逐位相同（11.3031082158203125 @ idx 129）**。
- 归因收敛：三种完全不同的内核（多掩码 store / runtime-stride / 纯连续
  双内核+torch 组装）+ guard 回退产出相同垃圾 ⇒ 常数性根因在内核之外：
  ①平台 case 3 值域不含于 [0,2^31)（guard 必然落入 64 位路径）；
  ②或 XMLIR 的 `view(torch.int32)` 小端重解释本身有错（所有轮共用的唯一
