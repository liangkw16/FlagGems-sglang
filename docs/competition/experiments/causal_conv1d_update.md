...
## e22 终态（2026-09-10，sub 12417）：宽度轴归约形态证伪，T43 收盘

- valid 但 avg **2.5084 大回退**（-62%）：天数 13.68→2.84、海光 10.91→3.91、
  沐曦 6.81→3.65、A 8.25→2.88、B 10.38→4.34（五芯 -46~-79%）；vendor 三芯
  不动持平。**他队沐曦 10-13 不是该形态**——scalar-FMA-strip generic 仍是
  GPU 类芯片最优。树已回滚 e21 字节。
- 负知识入账：常胜结构表 T43 行（宽度轴归约，"华为+燧原已验证"）**不可
  作为 generic 跨芯迁移**——GCU 上赢的形态在 GPU 类芯片上输一半以上；
  跨芯形态移植需逐芯读数证据，不能靠 vendor 族推断。
- **T43 收盘于 e21 6.6006（排名 4）**；今日唯一 Top1 攻坚尝试已按预注册
  门止损。燧原 0.48 vs 榜首 3.2、沐曦 6.8 vs 13.0 的结构面留待下一季。

## T43 Top1 可达性终判（2026-09-10T14:18，实时逐芯榜）

用户问"T43 还有戏不"。按 14:18 实时 leaderboard 三组核算，结论是
**Top1 数学上够不到，#3 理论可及但无弹药**，本批不再投入额度。

实时榜（我方 7.147，#4/5）与"对手已证明可达的同芯读数"：

| 芯 | 我方 | 对手最佳（队） | 若兑现 Δavg |
| --- | --- | --- | --- |
| 海光 | 11.10 | 15.89 (Fields) | +0.599 |
| 沐曦 | 9.60 | 13.06 (EvokeAgent) | +0.433 |
| 燧原 | 0.48 | 3.63 (EvokeAgent) | +0.394 |
| 华为 | 0.44 | 2.74 (Fields) | +0.288 |
| card_b | 10.23 | 11.42 | +0.149 |
| 天数 | 15.16 | 16.00 | +0.105 |
| card_a | 8.55 | 9.30 | +0.094 |

1. **组合下界**：越过 #3（7.903）需 Δ+0.757，最省组合是沐曦+燧原
   **两芯同时**兑现（→7.973）；夺 #1（8.550）需 Δ+1.404，最省是
   沐曦+燧原+海光**三芯同时**兑现（→8.571）。缺一即够不到。
2. **两根最肥的轴今日已实测封死**：燧原 0.48→3.63 属 persistent/pingpong
   形态，今日 T49/T57/T56/T42 四发平台实证全部噪声级，燧原结构轴全类关闭；
   沐曦/海光那根（宽度轴归约）即 e22，实测 avg −62%、五芯 −46~−79%，已回滚。
   **他队沐曦 13.06 不是该形态**，其结构未知。
3. **水位重掷天花板 7.3406**：把我方 30 次 T43 完成提交的逐芯历史峰值全部
   合成（天数 15.409 / 沐曦 9.598 / 海光 11.382 / card_b 10.756 / 华为 0.484 …）
   也只有 7.3406，**距 #3 仍差 0.56**——即便掷出各芯史上最佳水位名次也不动；
   且 e21 同字节重掷额度已在 e19r/e19r2/e21r2 用尽。

处置：本批剩余 2 发改投 T42/T53/T56 彩票轴（单芯撞窗即 #1，赔率结构优于
T43 的"多芯同时兑现"）。T43 收盘 **e21 6.6006，#4/5**；下一季的攻坚口是
燧原 0.48 与沐曦 9.60 两处结构差，需先取得他队形态情报。
