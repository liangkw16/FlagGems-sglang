...
## E6r2 终掷：8/8 valid 3.2161 未超 TB（2026-09-09T06:32，sub 11644）
- 带内，TB 3.2584 保持；同字节预算用尽。剩余身份 e7s。
## E7s 第二身份：8/8 valid 3.2038 未超 TB（2026-09-09T07:02，sub 11653）
- e7-stride 字节载体（ff373dd）带内；TB 3.2584 保持。T42 两身份重掷
  预算均尽，收盘守 TB（榜首 431 慢窗分，等极端窗证据才再议）。


## 2026-09-09 榜首性质更正：慢窗产物，此前打错靶

按题 leaderboard 端点（返回每支队伍逐芯 speedup，只读、不耗额度）判定：
本题榜首 **431.48** 的燧原读数为 **3427.0**，而同芯**第二名**仅 9.39
（aiaijiaolvle），相差 **365x**。按“榜首某芯 > 同芯次优 20 倍即窗口产物”
判据，该榜首是评测机慢窗下 reference 退化所致，**不是可复现的结构领先**。

据此更正本账本此前把该榜差记为“需同量级慢窗才能追平”的结论：

- **正确靶子是同芯次优 9.39，不是榜首 3427.0。** 我方燧原 1.700，
  该差距属结构可达范围。
- 对该彩票榜首继续投同字节重掷弹属错靶，会白烧全局额度（额度为账号
  全局每日 30 发、全题共享）。
- 仅把燧原抬到 9.39，八芯平均即 3.258 → 4.220。
