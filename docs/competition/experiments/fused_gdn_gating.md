...
## E4r 第二身份：8/8 valid 2.1853 未超 TB（2026-09-09T14:00，sub 11791）
- e4 字节载体（1813a97，旧字节+旧测试）带内（e4 原值 2.1267 上方），
  TB 3.0321 保持。T53 两身份预算均尽，收盘（榜首 288x 水位分，
  等极端窗证据）。
...
本题榜首 **288.43** 的燧原读数为 **2280.7**，而同芯**第二名**仅 7.84
（EvokeAgent），相差 **291x**。按“榜首某芯 > 同芯次优 20 倍即窗口产物”
判据，该榜首是评测机慢窗下 reference 退化所致，**不是可复现的结构领先**。

据此更正本账本此前把该榜差记为“需同量级慢窗才能追平”的结论：

- **正确靶子是同芯次优 7.84，不是榜首 2280.7。** 我方燧原 1.663，
  该差距属结构可达范围。
- 对该彩票榜首继续投同字节重掷弹属错靶，会白烧全局额度（额度为账号
  全局每日 30 发、全题共享）。
- 仅把燧原抬到 7.84，八芯平均即 3.032 → 3.804。
