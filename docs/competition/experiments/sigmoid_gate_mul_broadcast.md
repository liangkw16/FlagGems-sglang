...
- 从 e7 的 `[RB,W]` 布局试探两字节、`W≥1024` 宽行的
  `RB*W=131072`；fp32、窄行继续 65536。只改 `_enflame` 行块规模，
  添加 RB=128/12-CTA 越界和 fp32 控制回归。工作树候选源码 SHA-256
  `5c45d4f3cfff97d53959f9f746fa7dc4732d93a6718a397f5a51ce49b11ea262`，
  测试 SHA-256
  `21257feb33d8305ff10634b16299896839e2f3db713d9a91ee4563af74ab5a53`；
  未提交 Git，也未生成 ZIP。screening 回执
  `artifacts/competition/sigmoid_gate_mul_broadcast/e8-screen-20260923/verification.json`
  SHA-256 `9b31e7e3d3b25a6b39cba949078b806543b6e3f49010c25c22dadc05adce26ca`：
  NVIDIA 代理 5/5 方法、四路径执行，0 失败/错误/skip。
- wrapper-inclusive、五轮交替 AB/BA：`(129,1024)`、`(513,2048)`、
  `(513,5120)`、`(769,7168)` 的 e7/e8 中位速度比依次
  **0.215 / 0.347 / 0.400 / 0.618**；完整原始样本
  `e8-screen-20260923/bench.out` SHA-256
  `801e396d694332e727756c2478470fc227de635133e6f48759b7fca602a9aa73`。
  NVIDIA 不是 GCU 的性能证明，但四组稳定大幅退化，加上 T89 同宽度
  65536→131072 在 GCU 基本持平，未达到上膛筛选门。工作树源码与测试
  已恢复 e7 字节；**未 review、未打包、未提交平台**。
- `codex-ask`（`gpt-6-sol/max`）复核八芯榜差：冲榜首需总分增加
  4.4669，即均分 +0.5584；即使单独把燧原拉到榜首同芯 3.3937，
  均分也只到 2.7803。咨询建议暂停 T90 平台弹，转 T92；若重开，
  先在沐曦和另一个受影响目标芯测 generic 的 `(row,column_tile)`
  并行结构，至少两个非昆仑芯合计预期增加 1.2 芯分、无单芯 >5%
  回退后才考虑消耗额度。此门是决策阈值，不是已测收益。
