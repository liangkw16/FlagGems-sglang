...
| 69 | fused_moe_dispatch_index | invalid_correctness | submitted(13332,e1,5/8;燧原PassManager/华为off-by-one/昆仑崩溃族) | - | no | e2 候选就绪待提交（纯 1D 原语收窄：标量专家循环+逐lane名次+T64式标量gather，去全部 2D 形态与向量gather；release 全绿）；裁决点=燧原是否解除 PassManager、华为 off-by-one 是否消失、昆仑新 ZIP 重掷 | 2026-09-12 | [fused_moe_dispatch_index](fused_moe_dispatch_index.md) |
