...
## E4/E5 segment-owned 持久 worker（2026-09-04，submissions 9473/9479，daily_seq 17/18）

- E4（华为64/昆仑12 worker）：昆仑 0.2295→**0.6835（+198%，Codex 方向
  命中）**；华为 2.116→1.4895（-30% 回退）；均值 14.0178
- E5（昆仑保 segment-owned + 华为回退折叠字节）：华为恢复 2.085、
  昆仑保持 0.6375；但未动芯水位下漂（海光 26.6 / A 29.7 / B 17.2），
  均值 **13.7688 < E3 的 14.1052**——单芯 +0.4 的增益被 ±1.5 级水位
  噪声淹没
...
- 燧原最终 `执行超时(1830s)`（验证阶段子进程挂起）——**平台侧盒子
  忙，非代码失败**（同字节 vendor 在 E1/E3 窗口过线 0.33x）；T46 e6
