...
### E9 守榜重载(2026-08-31 04:2x CST)

- 触发:starwing 21.601x 夺冠(我方 E7 锚点 19.870);
- e9 = e8 燧原 vendor(+90% 单芯证据)重载体,冻结四成员与 E7
  逐字节一致——方差重掷(e8 低滚 16.6,E7 高滚 19.9,需 >21.6
  方能夺回);commit `4addcfd`,ZIP `9cd6862e…`;额度 21/30。

### E9 终态(sub 7058):守榜未果,T39 让冠

- 8/8 valid,avg 16.689(< E7 锚点 19.870,团队最佳不变)——
  第二次方差掷骰仍低滚(海光 35.5 vs E7 异常高值 56.4);
- 判定:E7 的 19.87 含海光单次异常高,**结构性真实水位 ~16.7-17**,
  starwing 21.6 为结构性优势;重载路径关闭(两滚皆 ~16.7)。
...
### E10 平台终态(sub 7480,2026-09-01 00:3x CST):机制坐实,均值输给异常锚点

preflight intent `0ce1c3f4…` 单次 confirm(sub 7480,额度 30→29/30);
远端回读 15352 bytes 与 canonical ZIP 一致(`verified`)。

终态 **8/8、valid、平均 `17.99791667x`、非 team best**(E7
`19.86983333` 保持——其海光 56.43 为异常高滚,E9 同字节复测已回
35.5,本轮 30.2):

| 芯片 | E7 | E10 | 变化 | 文件 |
| --- | ---: | ---: | ---: | --- |
| **华为** | 7.495 | **25.909** | **+246%(机制兑现)** | generic(新) |
| 天数 | 24.842 | 22.931 | -7.7% | generic(新) |
| 海光 | 56.428 | 30.247 | 水位回落(E9 已现 35.5) | generic(新) |
| 国际 A | 31.988 | 30.157 | -5.7% | generic(新) |
| 沐曦 | 27.318 | 22.935 | -16%(**冻结字节**,水位) | metax |
| 燧原 | 0.440 | 0.845 | +91%(冻结字节,水位回升) | enflame |
| 国际 B | 10.210 | 10.716 | +5%(冻结字节) | amd |
| 昆仑 | 0.238 | 0.243 | 持平(冻结字节) | kunlunxin |

**裁判**:

- 机制门(华为 +20%)以 **+246%** 兑现——token 块跳过在昇腾的
  发射吞吐瓶颈上引爆,为本批单芯最大结构性增益;specialize+注入
  预检的"零适配需求"信号与平台结果一致;
- 晋级门(>19.87)失败:均值损失集中在海光异常锚点回落(-26.2)
  与沐曦冻结字节水位(-4.4),非候选所致;对照 E9 结构水位
  16.69,e10 的 18.00 为 **+7.8% 结构性改善**;
- stop gate 字面触发(<18.87)但其因果前提(全满 shape 占比过高)
  被逐芯归因否定:generic 路由芯中华为 +246%,天数/国际 A 的
  -6~-8% 在已知噪声带内;
- **处置:树保留 e10 字节**(8/8 valid 且结构优于水位);E7 团队
...
### E11 平台终态(sub 7484,2026-09-01 00:5x CST):路由假设双芯证伪

preflight intent `297cc2d1…` 单次 confirm(sub 7484,额度 29→28/30);
远端回读 9766 bytes 与 canonical ZIP 一致(`verified`)。

终态 **8/8、valid、平均 `17.93854167x`、非 team best**:

| 芯片 | e10 | E11 | 变化 | 文件 |
| --- | ---: | ---: | ---: | --- |
| **国际 B** | 10.716 | **8.752** | **-18%(单轴门失败)** | generic(新路由) |
| **沐曦** | 22.935 | **20.320** | **-11%(单轴门失败)** | generic(新路由) |
| 华为 | 25.909 | 26.966 | +4%(冻结,水平确认) | generic(e10) |
| 海光 | 30.247 | 32.946 | 水位回升 | generic(e10) |
| 天数 | 22.931 | 23.002 | 持平 | generic(e10) |
| 国际 A | 30.157 | 30.401 | 持平 | generic(e10) |
| 燧原 | 0.845 | 0.881 | 持平(冻结) | enflame |
| 昆仑 | 0.243 | 0.240 | 持平(冻结) | kunlunxin |

**判定:双芯合计回退触发 stop gate,路由轴关闭。**

- **AMD vendor 结构性落后假设证伪**:card_b 在 generic 块跳过上比
  其专属 vendor 慢 18%——card_a/card_b 的 3 倍差距属硬件/水位,
  非结构;
- **沐曦 flat-full 优势再证**:metax 对本题确实偏好写满 padding 的
  flat-full 形态(E7 +65% 与本轮 -11% 互证),块跳过在沐曦不适用;
- 树已恢复 `_amd`/`_metax` E7 字节(`a662c810`/`dc6e0d1c` 复核一致),
  树终态 = e10 generic + E7 四 vendor(E8 燧原);
- **T39 收盘**:E7 `19.86983333x` team best 锁定;e10 结构字节保留
  在 generic(华为 27.0 水平两连证);已知轴尽:路由(e11)、方差
...
## E17:燧原 vendor 块跳过移植(2026-09-02 深夜预注册)

- 依据:全历史逐芯 oracle 仅 ~22.7 < 榜首 24.3,纯采样登顶不可能;
  唯一可加新纪录的轴 = 把 E10 的块跳过机制(华为 +246%)移植到从未
  试过的燧原 vendor(0.44-0.88)。结构 = E10 generic 逐字节同款
  (8 行 slab + 均匀标量分支 + 行级守卫),保留燧原 e8 已证
  BLOCK_COL 4096;已知风险:燧原"运行期分支"编译失败前科——失败
  零下行(团队最佳取 max),单发探针。
- 单变量:仅替换 `_enflame`;其余五成员冻结(E13 组合:E7 generic
  bdafd313 + ascend c005537c + amd a662c810 + kunlun 26980729 +
  metax dc6e0d1c)。
- 预注册门:基础门 8/8 valid 且燧原命中 `_enflame`;机制门燧原
  `>=0.9x`(历史最高 ~0.88);team-best 门平均 > E7 19.86983333。
...
### E17 终态(sub 8555,2026-09-03 00:4x CST):system_failed,轴永久关闭

- 燧原 `failed`(预注册的运行期分支编译约束应验:含运行时分支的结构
  在燧原编译失败,batch-2 知识扩及块跳过);七芯健康(海光 45.31/
  华为 27.53/其余在评)但无效分。stop gate 兑现:**燧原块跳过轴永久
  关闭**;树回滚 `_enflame` 至冻结 E8 算法字节(`2741c29c`)。
- 知识:燧原 runtime 分支编译约束与 kernel 规模无关(T31 微核同指纹、
  本题微分支直接编译失败)——**燧原上任何运行时分支结构都不可用**,
  一切跳过/谓词机制在该芯只能用 load-mask 形态。
- T39 登顶路径就此关闭(oracle 22.7 < 24.3 且无新纪录轴);维持 E7
  19.87 团队最佳,转为纯采样守位。
