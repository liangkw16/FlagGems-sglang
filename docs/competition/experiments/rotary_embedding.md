...
sealed: yes
next: 封存;仅燧原水位恢复信号时以 E10 字节重载(≤2 发,E10 ZIP 已验签在库)
updated: 2026-09-02
```
...
### E11 终态(sub 8264,15:5x CST):未破 E10,T35 封存

**8/8 valid,平均 5.90025,is_team_best=false(E10 7.047975 保持);
额度 17/30。**

| 芯片 | E10 | E11 | 归因 |
| --- | ---: | ---: | --- |
| huawei(t32→t16) | 1.4320 | **2.0160** | **+40.8%,t16 假设兑现**(华为峰值确认 t16) |
| enflame(同字节 t32) | 10.5928 | 0.9942 | **-90.6% 同字节塌回** → 触发预注册判定:t32"悬崖"是平台水位而非 tile 效应 |
| 其余六芯(冻结字节) | — | ±1% 内 | 天数 9.933/沐曦 6.2824/海光 9.3554/昆仑 0.296/card_a 9.6274/card_b 8.6976 |

- **燧原 tile 轴关闭**:t32 同字节跨窗塌回 t16 水位(~1.0),E10 的
  10.59 判水位尖峰;结合 08-31 T33 燧原同字节 6.28→0.88 前科,燧原
  读数不可作为单变量归因依据,必须同窗成对。
- 反事实核算:E11 窗口若按燧原尖峰口径,混合字节 ≈(5.90025×8 −
  0.9942 + 10.5928)/8 ≈ 7.10——方向与幅度同预注册期望 7.104 一致,
  混合假设本身成立,输在窗口水位。
- **T35 收盘于 E10 7.047975**(rank 8/14,距榜首 c2flow 12.7588
  -44.8%);树回滚 E10 字节(revert commit `990cd95`,ascend t32
  `2ddd1a7b` 复核一致)。
- 重载弹药:`e10-988f941/rotary_embedding.zip`
  (SHA-256 `e0630e66a0def46589dcd3aeae641a6be8b6abc64dc8b1e56c5fb561cc08bd86`)
  已验签在库;仅当燧原水位恢复信号(他队燧原读数跳升或我方同字节
  重载达标窗口)时重载 E10 字节,≤2 发,追高 team best。
- 跨芯知识沉淀:①燧原平台水位波动可达 10 倍量级(E10→E11 同字节
  10.59→0.99),燧原单变量结论必须同窗成对归因;②华为 HEADS_TILE
  峰值 16(4→8→16 单调,+128% 累计),32 回退;③当日双投同字节
  对照是水位判定的最小成本协议。
