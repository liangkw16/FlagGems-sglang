...
## e13r 判别终态：**分组形态 2/2 复现触发评测崩溃，轴判死**（2026-09-09T14:0x，sub 11756）

- e13r（`be00f43`，字节同 e13）燧原/昆仑**再次同指纹 1830s 验证执行
  阶段超时 + inductor compile_worker Aborted**（1830094/1833617ms）。
  同字节两连复现 = **新 kernel 触发评测侧资源问题**，非平台抖动。
- 处置：vendor 回滚 e12 字节（commit 在案）；分组单 launch 轴关闭；
  e14（dtype 探针）连带死，ZIP 留库不发。T47 结构弹药已耗 3 发
  （e12/e13/e13r），剩余只有窗口弹药（e5r2 尾窗在排）。
- **重要负知识入库**：分组单 launch GEMM（runtime work-loop + 平铺
  task 解码 + 多轮 K）在 NVIDIA 代理 12x，但 GCU/XPU 评测环境 2/2
  复现触发 eval 子进程崩溃——代理收益与平台可达性在这两芯上再次
  背离；S0 fused 形态、e13 分组形态两次证明：燧原/昆仑只吃 per-seg
  静态 launch 的规则 GEMM。榜首 42.98 的 c2flow 若无此限制，说明
  存在第三种我们未找到的形态。
