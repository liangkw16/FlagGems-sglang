...
## 第二阶段作战计划（2026-09-04 梳理，基于当日榜单刷新）

榜单快照：T42 榜首 3.5194（8 队过线）/ T43 6.8566（**仅 1 队**，76 发
提交）/ T45 17.4836（3 队）/ T46 18.7483（2 队）/ T47 23.3266（2 队）。
我队：T42 8/8·3.2489（差 8%）/ T46 8/8·14.105（差 33%）/ T47 7/8 封轴。

弹药观：额度不稀缺（12+30/日×6 天），约束是假设独立性与水位噪声
（T46 实测 ±1.5 均值噪声可淹没单芯 +0.4）。发弹门槛：单芯 ≥2x 或
正确性翻绿。

| 优先级 | 题 | 方案 | 判据 |
| --- | --- | --- | --- |
| P1 | T45 | **无 dot 结构 vendor**（Codex 处方）：燧原+昆仑逐 k 外积 FMA（纯 1D load + 广播乘加，零 dot/零转置 operand/零 2D 归约），华为附 64³ 低精度 Cube 单发 | 6/8→8/8（需燧原+昆仑翻绿且华为 >0.1x） |
| P2 | T43 | 回退 S0 七芯正确结构（e1 重构已证伪），昆仑 vendor 换 `x*tl.sigmoid(x)` lowering 形式（T36 昆仑实证 vs 我们的除法式），华为/燧原在 S0 骨架上做 BLOCK/warps 阈值冲刺（0.056/0.027→>0.1） | 任一弱芯翻绿或过线才续投 |
| P3 | T46 | 沐曦 segment-owned（镜像昆仑 +198% 结构）；其余水位赌博不发 | 单芯 ≥2x 才发 |
| 守 | T42/T47/T44 | T42 轴打满守榜；T47 昆仑 conclusive 封轴；T44 待情报 | — |

T43 情报备注：76 发仅 1 队过线说明全場都在盲试；我队 7/8 起点已是
第一梯队，P2 的单变量组合是当前唯一无情报可打的路径。
