...
> 第 1–13 节保留 2026-08-27 第二批复盘；第 14 节补充 2026-09-10 第四批经验与纠偏。
> 历史硬件判断、发布策略和额度策略仅代表当时记录，执行时以当前用户授权、项目规则及适用证据为准。

> 第二批范围:Task 08–24 全部 17 题(第一批未参与)。数据来源:17 份实验
...
(2026-09-17 第五批增量与订正见 §15:华为 persistent 谱系边界、GCU 钉高危/4096 峰、
后端可靠性矩阵新增三行。)
...
## 14. 第四批复盘与方法纠偏（2026-09-10）

本节根据比赛记录复盘，以第四批 Task 42–58 为主，结合前几批经验。
只使用已有本地资料，未刷新平台状态。成绩口径为
[2026-09-10 14:56:33 +08:00 快照](data/batch4-final-20260910-1440.json)：
17 题中 11 题有效，T51 `fla_layernorm_gated` 以 60.7192x 暂列 1/12；
这是当日提交投入结束时的快照，不是截止终榜或专家评审结果。

总体判断：已形成可追溯的跨芯开发、验证和提交闭环；主要短板是目标芯证据不足、
对性能差距归因过快，以及后期重复提交挤占结构优化与后续验证的预算。

### 14.1 有效经验：保留有逐芯证据的结构优化

- T43 改为通道连续布局后首次达到八芯有效。T58 组级 scale 累加在沐曦
  提升约 23.7%，但国际 B 回退约 22.9%，随后通过 vendor 分派处理。
- 先查数据布局、重复计算、访存和 launch 开销，再调 tile；收益必须逐芯判断。
  同族结构可作候选来源，不能把某题的一次成功升级为通用最优模板。

证据：[T43 账本](experiments/causal_conv1d_update.md)、
[T58 账本](experiments/w8a8_block_int8_matmul.md)。

### 14.2 正确性：复用上游必须连调用方前提一起读

T48 移植时遗漏上游调用方已切短 segment 的前提。BLOCK_M=64，却没有段内
多 tile 循环，长段直接漏算，出现 75% mismatch。这是实现和测试覆盖缺口，
不能据此否定完整分块方案。随后补齐 63/64/65/256 行段、vendor 覆盖，
并修复 11 个测试文件的 unittest 入口顺序。

下次锁定接口和 reference 后，重点检查 B-1/B/B+1、多 tile、非整除维度、
stride、dtype、舍入与输入不变性。失败 case 必须留下持久化回归。
证据：[T48 根因与回归](experiments/chunked_sgmv_shrink.md)。

### 14.3 验证：执行证据不能跨设备外推

NVIDIA 代理通过不证明目标芯的 lowering、数值或性能。KernelGen 返回
`passed=true`、测试数为 0，也不能计为目标芯正确性通过。
后续建立的 source/test/依赖/runner、实际入口与 kernel launch、日志、
ZIP 哈希绑定应继续保留；下一阶段优先补目标芯原失败输入与运行环境的复现能力。
证据：[09-06 流程实测](workflow-validation-20260906.md)。

### 14.4 归因：关闭具体候选，不能把假设写成架构定论

“未指定 num_warps 的提交分数较高”是相关性，不能直接认定 launch 参数是根因。
同一 persistent 模板在几题失效，只能关闭已测模板和参数范围，不能“永久关闭
整个燧原结构优化方向”。芯片经验应记录算子、shape、dtype、编译器版本和
证据范围，缺失项标为未知；重开条件是新源码、目标输入或运行环境证据。

同理，异常读数与较长评测耗时的相关性尚不能独立证明 reference 退化的机制。
从少量榜单最高值推测固定“夜间窗口”，存在采样与选择偏差，不应作为确定性排程依据。
本节纠正[终盘记录](strategy-batch4-last10-20260910.md)中“全类永久封死”
及固定窗口可预测性的过强表述。

### 14.5 评价：成绩与可复现的工程收益分别记账

T51 的燧原读数为 442.8042x，贡献了该次八芯平均分约 91%。快照第一名成立，
但全部增益不能归功于代码改动。另一方面，T42 即使避开极端榜首，第二名
燧原仍为 9.3332x，我方为 1.6846x，仍有待解释的差距。

开局即读取逐芯榜单，结合 reference 耗时、候选耗时和重复测量分析差距。
跨队差距用于提出假设，不能单凭分数反推出对方实现。
证据：[逐芯快照](data/batch4-final-20260910-1440.json)。

### 14.6 预算：重复测量需要收益门与执行约束

最后 10 发中，2 发验证结构，后 8 发重复测量仅带来 T42/T53 两项小幅新高，
没有换来名次提升。原计划把主要重复测量留到 16:30 以后，实际约 14:40 已耗尽
当日 30 次额度。计划与实际执行不一致是需要修正的流程问题。

历史最佳成绩保留不代表重复提交没有成本：每次仍占共享额度和时间。
应预先分配结构实验、必要复测及收尾预算，写清晋级和止损条件；预算调整记录理由。
现有证据不足以把未新增第一名归因于“没赶上夜间窗口”。
证据：[最后十发计划与实录](strategy-batch4-last10-20260910.md)。

### 14.7 评分与收尾：先有效，再增分，终态同步入账

T45 八芯正确、平均约 7x，仍因昆仑只有 0.063x、低于 0.1 门槛而无效。
先补正确性和门槛；全部达标后，按各芯加速比增量对算术平均的贡献选择候选，
不能只追最慢芯或某芯的相对提升百分比。

复盘时 CURRENT/INDEX 与原始快照存在不同步：T43 索引仍为 6.6006x，
快照已为 7.1472x。平台终态回执应同步写入 CURRENT 并生成索引，手写策略表
注明快照时间。本节仅记录差异，不冒充已完成各算子账本对账。
证据：[评分口径](README.md#3-评分与排名)、[实验索引](experiments/INDEX.md)、
[收盘快照](data/batch4-final-20260910-1440.json)。

### 14.8 下一批优先执行的三件事

1. 开局即做逐芯差距分析，优先争取有效题和有明确结构收益的题。
2. 每个实验先写假设、受影响芯片、晋级标准和止损条件，为必要复测保留有限预算。
3. 优先补目标芯复现能力与失败回归；每次提交都应增加可用证据，终态同步回填账本。

## 15. 第五批复盘（2026-09-17 收官日增量）

> 战斗全程与逐发判决见 [batch5-finalday 方案 §1-15](optimization-batch5-finalday-20260917.md)
> 与 [batch5-final-review](batch5-final-review-20260917.md)；本节只沉淀对 §2/§4/§5 的订正与补强。

### 15.1 战绩

收官日额度 30/30 用尽（本会话 28 发）：**7 题 12 次 TB 刷新、26 发 8/8 有效、
2 发平台侧无效、零自伤**。最大单项 T64 deepep_permute 7.171→**8.734**（+21.8%，
rank 14→9，六次刷新：去 clone 三段式→逆映射 gather→BLOCK 阶梯→燧原 4096）。
第五批终局 15/17 题有效（T60 燧原 torch-gcu int64 窄化为待核平台侧证据、
T70 昆仑驱动级挂死未解，两题账本留有重开条件）。

### 15.2 跨芯知识库增量（订正/补强 §2）

**华为 Ascend——persistent 谱系与边界（把 §2.3 的 T15/T16 证据扩展为 10 题谱系）**：

| 响应 | 任务（平台终态） |
| --- | --- |
| ✅ 宽逐元素/行散布 | T73 +97%、T64 +44%、T62 +36%、T67 +30%、T71 +19%、T75 +16% |
| ❌ slot 循环归约 | T65 -19%（40-program 双轴切分反而劣化） |
| ❌ 窄 lane flat scatter | T61 -19% |
| ➖ 已宽 copy / 除法 gather | T63 持平、T59 +2.9% |

→ 结论：`num_vectorcore` 封顶是**条件配方**而非普适优化；投放前按 kernel 族分类。
华为 index/散布族 7~23x 结构缺口当日穷尽可移植形态（persistent/行打包/SUB 批量
2-D store/gather）仍未破——需要授权主机 IR（EvokeAgent 在 T61 华为 6.50 反证
存在未知形态）。

**燧原 GCU（补强 §2.1）**：

- **任何 launch 钉高危**：warps=4 钉 -47%（T62 E11）、stages=3 钉 -46%（T62 E12）、
  纯净 launch 是默认最优——与 §2.1 "官方默认 +38%"（T19/T51/T68 三证）互为正反面。
- **gather/散布族 BLOCK 峰值 4096**：512 平→4096 **+42.6%**（T64 e13）→8192 -22%
  （过峰）；与 §2.1 已录 pointwise 32K 上限、reduction 8192 峰并列第三条 BLOCK 曲线。
- pingpong（num_stages≥3）只在特定循环形态生效：T68 两阶段循环 +39% vs T72
  scalar while 循环零效果——无法事前判定，一发探针。
- grid≤24 仅对超发形态（>10² program）有意义；T61 grid 本就个位数时是伪命题
  （且无 stride 循环时 cap 会破正确性——先读 kernel 再谈配方）。

**gather vs scatter（新事实）**：逆映射 gather（inv[dst]=token 预映射 + 纯行
gather + 连续 store）在 card_a/card_b/海光/沐曦/天数 +3~14%（T64 e9）；华为
gather -7%（两者皆非其突破口）。去 clone 三段式的流量算术：(1+3K)·T·H →
(1+K)·T·H，K=8 时代理 2.2x、平台 +4~14%（双路径分发消除小 shape 回退）。

**后端可靠性矩阵新增**（并入 §2.8 使用习惯）：

| 事实 | 证据 |
| --- | --- |
| 2-D `tl.cumsum`/`tl.sum` 数值错 @沐曦+card_b | T69 E14（masked_m 计数偏差 3%） |
| **同字节编译判定漂移 @昆仑**（平台栈赛中会变） | T66 e3：generic 字节 09-16 八芯过、09-17 `linalg.matmul` 形状断言挂 |
| warp 聚合原子 @天数证伪 | T69 E14：77→66.6（"逐 lane 原子是瓶颈"假设错） |

### 15.3 方法论增量（补强 §4）

1. **同芯次优可达性判定**：`sum(每芯全场次优)/n` vs 榜首均值——客观回答"哪题能
   冲第一"。本日唯一 >1 的是 T64（1.29x）；榜首单芯彩票识别后真实目标=同芯次优。
   开题/排弹药必先跑这张表（脚本沉淀在 finalday 方案 §1 的快照流程里）。
2. **流量算术先于编码**：wrapper clone/多余 pass 的字节占比分析直接给出结构方向，
   比代理 benchmark 更早锁定 T64 的去 clone 主攻。
3. **窗口体制相关性**：多芯窗口同涨同落（T63 e21：判据芯全守住但 4 芯同步回落
   -5~-24%）——窗口不是独立噪声而是整机体制。水位载体纪律（同执行字节新 ZIP、
   榜上恒取 max、低滚 2 即停）四连实战全部保底、一次未中——按 30% 胜率游戏规划。
4. **send 中断恢复路径**（两次实战）：status 核对无提交记录+额度未扣 → 用户授权
   归档 sending intent → 重 preflight 单发。T64 e11 中断重发后拿下新 TB。
5. **并行会话额度共享**：30/日是账号全局，多会话并行时互相消耗（本日并行会话用
   2 发）；排弹药按全局预算留余量。
6. **回执门禁的自证价值**：载体开发中漏改 tiles 基准（BLOCK 4096/tiles 8192 错配）
   被 release 全量矩阵当场拦下（column 4096 精确失配）——"同执行字节"声明必须由
   回执验证，不能靠口头/眼睛保证。

### 15.4 反模式增量（补强 §5）

- **配方不可跨 kernel 族盲推**：persistent 三个反例（T65/T61/T63）都是"看起来
  该有效"的题——投放前先分类 kernel 形态（§15.2 谱系表）。
- **窗口算术的乐观偏差**：判据芯守住≠其他芯守住；预期值按"窗口体制"整体波动
  而非单芯外推。
- **官方教程可移植子集≠完整形态**：SUB 批量 2-D store（-35%）≠ insert_slice
  完整 UB 组装；移植前先查 API 存在性（`tl.insert_slice` 不在 Triton 3.7.1 主线）。

### 15.5 下一季清单补丁（衔接 §8）

1. 华为 index 族 7~23x：优先申请授权昇腾主机做同源 IR/profile 对照（T65 诊断
   文档已备实验设计）；在此之前不要在 launch 级再花弹药。
2. 燧原 2~18x 结构差疑似完整 UB 组装形态——开赛第一周先探测平台 Triton 版本的
   extension API 面。
3. 昆仑平台栈会赛中漂移：同字节重投前预期编译判定可能变化，账本记录字节→判定
   的时间对（T66 案例留档）。
4. 逐芯情报快照（`tools`+leaderboard 端点）开赛日即建立，每次排弹药前重算可达性表。
