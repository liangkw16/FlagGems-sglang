...
## 2026-09-20 白昼 D4 备战咨询（codex-ask 第六轮，四项纠错已核实）

> 双重门通过：wave-2 commit `f3b8cad1` 深度 review（high effort）零缺陷。

### 已核实的纠错（写入决策）

1. **T91 算术修正**：榜差 0.079×8 = **0.632 芯分**；沐曦 +0.5 只到
   2.0155 < 2.032——**还差 0.132 芯分**（华为 0.8→0.9 = 0.1 +
   燧原 0.7→0.9 = 0.2 可补）。T91 e8 仍发但预期为"接近弹"非"必下弹"。
2. **T89/T90 数学上限**：即使燧原 4.0 + 华为 1.8 全兑现，T89 也只
   ~2.83 < 3.17；T90 华为 2.1 只到 ~2.63 < 3.21——**wave-2 是 TB 弹
   不是 Top1 弹**（四发全过门仍不足 Top1 属正常，不据此否定机制）。
3. **T83 中点测试错**：bf16 在 1.0 附近间距 2^-7、真中点增量 2^-8；
   现测试用 2^-9（1/4 间距，恒舍回 1.0，无 tie 覆盖）——测试声称的
   "中点"从未被测过。修正后作为三处舍入（one_plus/scaled/final）定位
   诊断的一部分。
4. **T77 探针判据缺口**：stage3/4 异常后仍返回 0——exit=0 不能作为
   探针通过判据；且 stage4 未验证 Triton 算术跨 2^31。无主机期间
   维持封存。

### D4 白昼优先级（咨询采纳）

1. **T92 直接 batch 连续段拷贝**（第一优先结构轴）：题面 cu/seq_lens
   直接表达"每 batch 一段连续 H×D 拷贝"，省掉 searchsorted 行映射
   全部框架开销；内核内标量段起点+连续偏移，无张量索引 gather。
2. **T90 行任务划分**：少行长行横向 (row, hidden_tile) vs 多行短行
   合并——按慢的一类单方向做。
3. **T89 循环/访存生成诊断**：不再扩大 BLOCK；用有效带宽
   4×numel/耗时 判断是否搬运受限。
4. **T92 榜首 349.9 判读**：18.4% 涨幅不是窗口证据；先对照榜首两次
   提交八芯分布+我方回执 wrapper 占比，慢窗复测维持预注册门。

### 09-21 额度分段（咨询采纳）

午夜 4（已武装）→ 四发终态后 6（正信号组合/根因修复）→ 白昼 8
（T92/T90/T89 新轴）→ 晚间 6（兑现+窗口复测≤2）→ 应急 6。
